...
import { c as createExpense } from "./expensesApi-Bq0211JH.js";
...
const CreateExpense = () => {
  ...
  const createExpenseMutation = useMutation({
    mutationFn: (values) => createExpense(values),
    onError: (error) => {
      console.log(error);
    },
    onSuccess: () => {
  ...
  const validationSchema = create$3().shape({
    ...
    amount: create$5().required("Amount is required").min(0, "Amount cannot be negative").max(1e11, "Amount cannot exceed 100 billion").typeError("Amount be a number"),
