import { f as useQueryClient, b as useLocation, r as reactExports, a as useNavigate, u as useToast, j as jsxRuntimeExports } from "./index-BwJeGL0w.js";
...
import { a as fetchOneReceipt, b as updateReceipt } from "./receiptApi-DfxsCQnj.js";
...
const UpdateReceipt = () => {
  ...
  const [sizeType, setSizeType] = reactExports.useState("none");
  ...
  reactExports.useEffect(() => {
    if (data) {
      setInitialValues({
        ...
        receipt_date: data?.receiptDate ? hooks(data?.receiptDate).format("YYYY-MM-DD") : "",
        inventory: data?.item?.map((inv) => ({
          id: inv?.id,
          name: inv?.item_name,
          ...
          item: {
            ...
            unit: inv?.unit,
            size: inv?.size,
            expiryDate: inv?.expiryDate ? hooks(inv?.expiryDate).format("YYYY-MM-DD") : ""
          }
        }))
      });
    }
  ...
  const updateReceiptMutation = useMutation({
    mutationFn: (values) => updateReceipt({ ...values, id: state.id }),
    onError: (error) => {
      console.log(error);
    },
    onSuccess: () => {
  ...
  const validationSchema = create$3().shape({
    source: create$6().required("Sourse is required"),
    issuanceDirective: create$6().required("issuanceDirective is required"),
    receipt_date: create$6().required("Receipt Date is required"),
    inventory: create$2().of(
      create$3().shape({
        id: create$6().nullable(),
        name: create$6().required("Name is required"),
        sizeType: create$6(),
        inventoryId: create$6().nullable(),
        itemId: create$6().nullable(),
        item: create$3().shape({
          id: create$6().nullable(),
          itemId: create$6().nullable(),
          location: create$6().required("Inventory Location is required"),
          quantity: create$5().required("Inventory Quantity is required").typeError("Quantity must be a valid number"),
          price: create$5().required("Inventory Price is required").typeError("Price must be a valid number"),
          amount: create$5().required("Inventory Amount is required").typeError("Amount must be a valid number"),
          unit: create$6().required("Inventory Unit is required"),
          size: create$6().required("Inventory Size is required"),
          expiryDate: create$6()
        })
      })
    )
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(jsxRuntimeExports.Fragment, { children: [
    ...
    /* @__PURE__ */ jsxRuntimeExports.jsxs("div", { className: "flex flex-row justify-between pb-4", children: [
      /* @__PURE__ */ jsxRuntimeExports.jsx(Header, { title: "Update Receipt", description: "Receipt" }),
